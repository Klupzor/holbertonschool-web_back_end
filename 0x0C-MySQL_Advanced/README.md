...
Write a SQL script that creates a stored procedure `ComputeOverallScoreForUser` that computes and store the overall score for a student.

**Requirements:**

-   Procedure `ComputeOverallScoreForUser` is taking 1 input:
    -   `user_id`, a `users.id` value (you can assume `user_id` is linked to an existing `users`)
...
Enter password:
bob@dylan:~$
bob@dylan:~$ cat 7-overall_score.sql | mysql -uroot -p holberton
Enter password:
bob@dylan:~$
